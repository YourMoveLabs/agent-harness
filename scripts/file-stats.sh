#!/bin/bash
# file-stats.sh — Codebase metrics for Tech Lead agent.
...
# --- Build find patterns ---
# Scan source directories. Override with SOURCE_DIRS env var (colon-separated).
if [[ -n "${SOURCE_DIRS:-}" ]]; then
    IFS=':' read -ra SEARCH_DIRS <<< "$SOURCE_DIRS"
else
    # Auto-detect common source directories
    SEARCH_DIRS=()
    for candidate in api src backend frontend/src scripts; do
        [[ -d "$PROJECT_ROOT/$candidate" ]] && SEARCH_DIRS+=("$PROJECT_ROOT/$candidate")
    done
fi
